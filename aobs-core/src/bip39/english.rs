//! The BIP-39 English wordlist, and nothing else.
//!
//! English only in v1 (`02-core.md` §4): CJK is excluded by ~100 MiB of glyph coverage
//! against a 21 MiB stack, and the Latin lists need diacritics the pinned `us` keymap
//! cannot type.
//!
//! Generated from <https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt>.
//! `bip39_tests.rs` asserts the SHA-256 of the list, joined with newlines, against the
//! digest BIP-39 itself publishes — which is how a word mangled by an edit here fails a
//! test instead of deriving somebody else\'s wallet.

/// The 2048 words, in BIP-39 order. The index is the 11-bit value the word encodes.
///
/// `static` rather than `const`: a `const` array of this size is copied into every use site.
