...
REQUIRED_BUILT_IN = {
    ...
    "FB_VESA": (
        "the BIOS firmware framebuffer, which is the console on a machine whose firmware offers a "
        "VESA linear mode - not the target machine, where it never bound"
    ),
    "FB_SIMPLE": (
        "simplefb, which is the console on the one machine this project has booted: coreboot "
        "registers a `simple-framebuffer` device from its own table, and nothing else registered "
        "a framebuffer there at all"
    ),
