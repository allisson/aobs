...
| Address-search time across 4 accounts × 2 branches × 1000 indices (8,000 derivations). **Instrumented, and read once off a dev machine: 246 ms** (release, Apple silicon) — the appliance now prints `AOBS_SEARCH window=8000 matched=yes\|no elapsed-ms=…` on every search, so what is left owed is that line on target hardware. It is the one row on this table the appliance can answer that CI still cannot, because QEMU has no synthetic UVC device to reach the screen with. | [#21](https://github.com/allisson/aobs/issues/21), [#27](https://github.com/allisson/aobs/issues/27), [#83](https://github.com/allisson/aobs/issues/83) |
